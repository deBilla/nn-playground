...
export function FightClub() {
  ...
  const loop = useCallback(() => {
    ...
    if (state.runState === 'running') {
      ...
      if (spd <= 3) {
        // Slow mode: only step every N frames so moves are visible
        // speed 1 = 1 step every 6 frames (~10 steps/sec)
        // speed 3 = 1 step every 2 frames (~30 steps/sec)
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="w-72 border-r border-slate-800 overflow-y-auto p-4 flex flex-col gap-4 bg-slate-900/50">
        <InfoBanner title="Robot Fight Club" variant="info" defaultOpen={true} dismissable>
        ...
        <InfoBanner title="How it works" variant="legend" defaultOpen={false}>
          ...
          <p>Over time they learn: approach → attack when in range → block when opponent attacks → dodge kicks.</p>
        </InfoBanner>
      </div>
